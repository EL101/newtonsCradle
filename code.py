...
input_text = text(text='Choose Mode of Input', align='center', color=color.red, visible=True)
click_drag_button = button(bind=click_drag, text="Click and Drag")
manual_button = button(bind=manual, text="Manual Input")

while mode==0:
    rate(program_rate)

input_text.visible=False
click_drag_button.delete()
manual_button.delete()

num_balls_slider_caption = wtext(text = "Set Total # of Balls:")
num_balls_slider = slider(bind=setNumBalls, min=1, max=10, value=num_balls, step=1)
num_balls_text = wtext(text = "# of Balls: "+str(num_balls_slider.value))
scene.append_to_caption('\n\n')

if mode==2:
    initial_angle_slider_caption = wtext(text = "Set Initial Angle:")
    initial_angle_slider = slider( bind=setInitialAngle, min=0, max=90, value=initial_angle, step=1)
    ...
    num_starting_balls_slider = slider(bind=setNumBallsStarting, min=1, max=10, value=num_starting_balls, step=1)
...
if mode==1:
    ...
    click_drag_text = text(text='Click and Drag', pos=vec(0,3,0), align='center', color=color.red, visible=True)
    ...
    for i in range(0, num_starting_balls):
        balls[i].pos=pos=vec(L * sin(initial_angle)+2*i*ball_radius+offset,axis_pos-L * cos(initial_angle),0)
        rods[i].pos=balls[i].pos
        rods[i].axis=vec(2*i*ball_radius+offset,axis_pos,0)-balls[i].pos
        balls[i].theta=initial_angle
...
print(num_balls, num_starting_balls, initial_angle)
while (True):
    rate(program_rate)
    if !run:
        continue
